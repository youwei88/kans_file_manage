...
<body>
	...
	<div class="easyui-panel" style="padding:5px">
		...
		<ul id="tt" class="easyui-tree" data-options="url:'${base}/user/getDirs?isFromSap=0',method:'get',animate:true"></ul>
	</div>
	<div id="addName" class="easyui-dialog" modal="true" closed="true" style="padding:5px;width:200px;height:120px;" title="添加目录">
		目录:<input id="dirName" name="dirName" />
		<button onclick="add()">确定</button>
	</div>
	
	<div id="addRootDir" class="easyui-dialog" modal="true" closed="true" style="padding:5px;width:200px;height:180px;" title="添加目录">
		目录:<input id="rootDirName" name="rootDirName" />
		描述:<input id="operName" name="operName" />
		<button onclick="add()">确定</button>
	</div>
		...
		function add() {
			...
			if (name){
				$.ajax({
					url:"<%=base%>/dir/addDir",
			        type:"POST",
			        data: {parentId:parentId,name:name,path:path,isFromSap:0,operName:operName},
			        success:function(data){
			        	if($('#addRootDir').dialog('open')){
			        		$('#addRootDir').dialog('close');
			        	}
